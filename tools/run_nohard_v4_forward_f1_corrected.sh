...
container_name=nohard-v4-forward-f1-corrected-c15850-200
parent_run=Sep15_01-55-29_nohard_v4_fromzero_17200chain_low065_filter6speed_arm002_seed33_c15750_to16050
run_name=nohard_v4_forward_f1_corrected_oldrecipe_from_upward_c15850_200
...
docker run -d --name "$container_name" --gpus all --ipc=host --shm-size=16g \
  ...
  --max_iterations 200 --save_interval_override 25 \
  ...
  --zero_latency_probability_override .35 \
  --max_latency_probability_override .30 \
  ...
  --command_pos_dx_min_override 0 \
  --command_pos_dx_max_override .30 \
  ...
  --velocity_cost_enabled 1 \
  --velocity_cost_source takeoff_tail_overspeed \
  --velocity_cost_limit .005 \
  --velocity_cost_dual_lr .01 \
  --velocity_cost_lambda_init 1 \
  --velocity_cost_free_band .8 \
  --velocity_cost_full_ratio .9 \
  --run_name "$run_name" \
  --effective_config_output /workspace/project/artifacts/velocity_recovery_20260913/nohard_v4_forward_f1_corrected_c15850_200_effective.json
